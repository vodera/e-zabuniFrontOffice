...
/**
 * @ngdoc function
 * @name CMSAdminApp:AllCtrl
 * @description
 * # AllCtrl
 * Controller of the CMSAdminApp
 */
...
//                reading meta header value
                    
                console.log("_csrf: ", $scope.csrf_value);
                console.log("_csrf_header: ", $scope.csrf_header);

                $scope.resetFilter = function () {
                    $scope.DashboardFilter = {};
                    var startDate = new Date();
                    startDate.setDate(1);
                    $scope.DashboardFilter.dateFrom = startDate;
                    $scope.DashboardFilter.dateTo = new Date();
                    $scope.loadCollectionSummary(true);
                };

                $scope.donutChart = Morris.Donut({
                    element: 'pie-chart',
                    data: [
                        {label: "Offline Requests", value: 0}]
                ...
                $scope.barChart = Morris.Bar({
                    ...
                    barColors: function (row, series, type) {
                        ...
                        if (row.label === "Crew Scheduled")
                            return "#AD1D28";
                        else if (row.label === "Crew Confirmed")
                            return "#00AFD0";
                        else if (row.label === "Initiated")
                            return "#4A9DCA";
                        else if (row.label === "Forwarded")
                            return "#00AD9A";
                        else if (row.label === "Tallied")
                            return "#F19332";
                        else if (row.label === "Posted")
                            return "#75B749";
                    }
                });

                $scope.showDateFilter = function () {
                    $scope.dtrange = !$scope.dtrange;
                };

                $scope.loadDailySummary = function (numDays) {
                    $scope.progressBar = $rootScope.showProgress();
                    $scope.selectedDays = numDays ? numDays : 10;
                    DataService.getDailySummary($scope.selectedDays).then(function (response) {
                        console.log("Records:", response);
                        $scope.DailyRequests = response.data.data.content;

                        $scope.records = [];
                        console.log($scope.records);
                        var today = new Date();

                        for (var i = ($scope.selectedDays - 1); i >= 0; i--) {
                            if (i === ($scope.selectedDays - 1)) {
                                today.setDate(today.getDate() - i);
                            } else {
                                today.setDate(today.getDate() + 1);
                            }
                            var index = $scope.DailyRequests.map(function (e) {
                                return e.hour.substring(0, 10);
                            }).indexOf($filter('date')(today, 'yyyy-MM-dd'));
                            var recordCount = index > -1 ? $scope.DailyRequests[index].recordCount : 0;
                            var entry = {numRecords: recordCount, date: $filter('date')(today, 'yyyy-MM-dd')};
                            $scope.records.push(entry);
                        }

                        $scope.areaChart.setData($scope.records);
...
//                loading customer summary automatically
                DataService.getCustomerSummary().then(function (response) {
                    console.log("Customer Summary:", response);
                ...
                $scope.loadCollectionSummary = function (reset) {
                    $scope.dtrange = false;
                    $scope.progressBar = $rootScope.showProgress();
                    DataService.getCollectionSummary($scope.DashboardFilter).then(function (response) {
                        console.log("Cash Collection Summary:", response.data);
                        $scope.CashCollectionSummary = response.data.data;

                        angular.forEach($scope.CashCollectionSummary, function (value, key) {
                            $scope.CashCollectionSummary[key] = key === 'totalAmount' ? $scope.unitfy(value, '2') : $scope.unitfy(value, '0');
                        });

                        var data = [
                            {y: "Initiated", a: $scope.CashCollectionSummary.initiated},
                            {y: "Forwarded", a: $scope.CashCollectionSummary.forwarded},
                            {y: "Crew Scheduled", a: $scope.CashCollectionSummary.crewScheduled},
                            {y: "Crew Confirmed", a: $scope.CashCollectionSummary.crewConfirmed},
                            {y: "Tallied", a: $scope.CashCollectionSummary.tallied},
                            {y: "Posted", a: $scope.CashCollectionSummary.posted}];
                        $scope.barChart.setData(data);

                        $scope.donutChart.setData([{label: "Offline Requests", value: $scope.CashCollectionSummary.offline}]);
                        $scope.progressBar.close();
                        if (reset) {
                            $scope.loadDailySummary();
                        }
                    }, function (error) {
                        console.log("Error", error);
                        $scope.progressBar.close();
                ...
                $scope.unitfy = function (data, fractionSize) {
                    if (data >= 1000000000) {
                        return $filter('number')(data / 1000000000, fractionSize) + 'B';
                    } else if (data >= 1000000) {
                        return $filter('number')(data / 1000000, fractionSize) + 'M';
                    } else if (data >= 1000) {
                        return $filter('number')(data / 1000, fractionSize) + 'K';
                    } else {
                        return $filter('number')(data, fractionSize);
                    }
                };
...
//                
        ...
        .controller('CustomerCtrl', ['$rootScope', '$scope', 'DataService', '$localStorage', '$window', '$dialogConfirm', '$filter', function ($rootScope, $scope, DataService, $localStorage, $window, $dialogConfirm, $filter) {
                ...
                $scope.loadCustomerCategories = function () {
                    $scope.progressBar = $rootScope.showProgress();
                    DataService.getCustomerCategories(true).then(function (response) {
        ...
        .controller('ListCustomerCtrl', ['$rootScope', '$scope', 'DataService', '$window', '$localStorage', function ($rootScope, $scope, DataService, $window, $localStorage) {
...
//                $scope.outletChange = function (item) {
//                    $scope.outlet = item.
//                };
...
//                };
                ...
                $scope.viewOutlet = function (CustomerOutlet) {
                    ...
                    DataService.OutletAccounts(CustomerOutlet.outletId).then(function (response) {
                        console.log("Outlet Accounts:", response.data.data);
                        $scope.outletaccs = response.data.data;
                ...
                $scope.linkNewAccount = function () {
                    ...
                    DataService.addNewCustomerAccount($scope.LinkCustAcc).then(function (response) {
                        console.log("Link Account resp:", response.data.data);
                        $scope.progressBar.close();
                        $scope.closeAndclear();
...
//                    }
                ...
                $scope.saveCITCrew = function () {
                    if (!$scope.citAgentCrewForm.$valid) {
                        return;
                    }
                    ...
                    DataService.saveCITAgentCrew($scope.CITAgentCrew).then(function (response) {
                        ...
                        $window.location = '#/ApproveCITCrew';
                    }, function (error) {
                        console.log("Error", error);
                        $scope.progressBar.close();
                        if (error.status === -1) {
...
//                    $scope.MessageFilter.dateTo = new Date();
        ...
        .controller('SystemUserCtrl', ['$rootScope', '$scope', 'DataService', '$window', '$timeout', '$q', function ($rootScope, $scope, DataService, $window, $timeout, $q) {
                ...
                $scope.userTypeChange = function () {
                    switch ($scope.SystemUser.userType) {
                        case "Bank Operator":
...
//                $scope.UserFilter = {};
        ...
        .controller('ListSystemUserCtrl', ['$rootScope', '$scope', 'DataService', '$window', '$localStorage', function ($rootScope, $scope, DataService, $window, $localStorage) {
                ...
                $scope.saveSystemUser = function () {
                    ...
                    if($scope.SystemUser.departmentId){ SystemUserUpdate.departmentId = $scope.SystemUser.departmentId;}
...
//                        Scroll top of page
        ...
        .controller('DepartmentCtrl', ['$rootScope', '$scope', 'DataService', '$routeParams', '$localStorage', '$window', function ($rootScope, $scope, DataService, $routeParams, $localStorage, $window) {
                ...
                $scope.saveDepartment = function () {
                    ...
                    DataService.saveDepartment($scope.Department).then(function (response) {
                        ...
                        $scope.progressBar.close();
                        $scope.ShowDepartmentDlg = false;
                        if ($scope.Department.regionId !== undefined) {
        ...
        .controller('PasswordPolicyCtrl', ['$rootScope', '$scope', 'DataService', '$routeParams', '$localStorage', '$window', function ($rootScope, $scope, DataService, $routeParams, $localStorage, $window) {
                ...
                $scope.savePasswordPolicy = function () {
                    ...
                    $scope.progressBar = $rootScope.showProgress();
                    $scope.PasswordPolicy.expiry = $scope.PasswordPolicy.expiry * (60 * 24);
                    $scope.PasswordPolicy.otpExpiry = $scope.PasswordPolicy.otpExpiry * (60);
                ...
                $scope.loadPasswordPolicy = function () {
                    ...
                    DataService.getPasswordPolicy().then(function (response) {
                        ...
                        console.log(response.data.data.otpExpiry);
                        $scope.PasswordPolicy.expiry = $scope.PasswordPolicy.expiry / (60 * 24);
                        $scope.PasswordPolicy.otpExpiry = $scope.PasswordPolicy.otpExpiry / (60);
                        $scope.progressBar.close();
                        $window.scrollTo(0, 0);
